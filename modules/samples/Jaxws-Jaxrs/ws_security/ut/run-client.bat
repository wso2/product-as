...
cd %WSO2AppServer_HOME%\samples\Jaxws-Jaxrs\ws_security\ut
set CLIENT_CLASSPATH=.\conf;.\temp\classes
FOR %%C in (..\..\..\..\samples\Jaxws-Jaxrs\ws_security\ut\temp\dependencies\*.jar) DO set CLIENT_CLASSPATH=!CLIENT_CLASSPATH!;..\..\..\..\samples\Jaxws-Jaxrs\ws_security\ut\temp\dependencies\%%~nC%%~xC
...
%_RUNJAVA% %JAVA_OPTS% -Dwso2appserver.home="%WSO2AppServer_HOME%" -cp "%CLIENT_CLASSPATH%" -Djava.endorsed.dirs="%WSO2AppServer_HOME%\lib\endorsed";"%JAVA_HOME%\jre\lib\endorsed";"%JAVA_HOME%\lib\endorsed" demo.wssec.client.Client src/main/resources/hello_world_wssec_client.wsdl %*
cd %CURRENT_DIR%
endlocal
:end
