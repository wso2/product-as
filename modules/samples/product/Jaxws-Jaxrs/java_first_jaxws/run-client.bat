...
cd %WSO2AppServer_HOME%\samples\Jaxws-Jaxrs\java_first_jaxws
set CLIENT_CLASSPATH=.\conf;.\temp\classes
FOR %%C in (..\..\..\samples\Jaxws-Jaxrs\java_first_jaxws\temp\dependencies\*.jar) DO set CLIENT_CLASSPATH=!CLIENT_CLASSPATH!;..\..\..\samples\Jaxws-Jaxrs\java_first_jaxws\temp\dependencies\%%~nC%%~xC
...
%_RUNJAVA% %JAVA_OPTS% -Dwso2appserver.home="%WSO2AppServer_HOME%" -cp "%CLIENT_CLASSPATH%" -Djava.endorsed.dirs="%WSO2AppServer_HOME%\lib\endorsed";"%JAVA_HOME%\jre\lib\endorsed";"%JAVA_HOME%\lib\endorsed" demo.hw.client.Client http://localhost:9763/java_first_jaxws/services/hello_world %*
cd %CURRENT_DIR%
endlocal
:end
