...
cd %WSO2AppServer_HOME%\samples\EntitlementFilter
set CLIENT_CLASSPATH=.\conf;.\temp\classes
FOR %%C in (..\..\samples\EntitlementFilter\temp\dependencies\*.jar) DO set CLIENT_CLASSPATH=!CLIENT_CLASSPATH!;..\..\samples\EntitlementFilter\temp\dependencies\%%~nC%%~xC
...
%_RUNJAVA% %JAVA_OPTS% -Dwso2appserver.home="%WSO2AppServer_HOME%" -cp "%CLIENT_CLASSPATH%" -Djava.endorsed.dirs="%WSO2AppServer_HOME%\lib\endorsed";"%JAVA_HOME%\jre\lib\endorsed";"%JAVA_HOME%\lib\endorsed" org.wso2.carbon.entitlement.filter.client.Client http://localhost:9764/ %*
cd %CURRENT_DIR%
endlocal
:end
