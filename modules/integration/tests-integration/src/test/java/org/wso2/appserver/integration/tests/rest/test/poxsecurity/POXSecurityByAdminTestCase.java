...
public class POXSecurityByAdminTestCase extends ASIntegrationTest {
    ...
    public void initialize() throws Exception {
        ...
        adminUserName = asServer.getSuperTenant().getTenantAdmin().getUserName();
        adminPassword = asServer.getSuperTenant().getTenantAdmin().getPassword();
    }
    ...
    public void testAddNewStudent() throws IOException,
        ...
        HttpsResponse getResponse =
                HttpsURLConnectionClient.getWithBasicAuth(studentGetUri, null, adminUserName, adminPassword);
    ...
    public void testDeleteStudent() throws IOException, LoginAuthenticationExceptionException,
            XMLStreamException, XPathExpressionException {
        String securedRestURL = getSecuredServiceEndpoint(SERVICE_NAME) + "/student/" + studentName;
        HttpsResponse response =
                HttpsURLConnectionClient.deleteWithBasicAuth(securedRestURL, null, adminUserName, adminPassword);
        assertTrue(!response.getData().contains(studentName)
                , "response doesn't contain the expected output");
    }
