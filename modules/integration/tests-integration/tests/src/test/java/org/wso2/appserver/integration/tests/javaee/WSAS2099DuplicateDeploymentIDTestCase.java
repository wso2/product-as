...
public class WSAS2099DuplicateDeploymentIDTestCase extends ASIntegrationTest {
    ...
    public void deployDuplicateWebappTest() throws Exception {
        if (userMode == TestUserMode.SUPER_TENANT_ADMIN) {
            String[] carbonLogs = ASIntegrationLoggingUtil.getLogsFromLogfile(new File(System.getProperty(
                    "carbon.home") + File.separator + "repository" + File.separator + "logs" + File.separator +
                    "wso2carbon.log"));
            boolean isLogInLogsRecorded = false;

            for (String logEvent : carbonLogs) {
                if (logEvent.contains("Application cannot be deployed as it contains deployment-ids which are in " +
                        "use")) {
                    isLogInLogsRecorded = true;
                    break;
                }
            }
            assertFalse(isLogInLogsRecorded, "Application cannot be deployed as it contains deployment-ids which are " +
                    "in use " + userMode);
        }
    }
}
