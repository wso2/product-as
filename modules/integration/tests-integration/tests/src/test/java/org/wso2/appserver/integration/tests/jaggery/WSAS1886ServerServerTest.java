...
/**
 * This class sends requests to serverServer/index.jag and validates the response
 */
public class WSAS1886ServerServerTest extends ASIntegrationTest {

    private static final Log log = LogFactory.getLog(WSAS1886ServerServerTest.class);
    private final String hostName = "localhost";

    @BeforeClass(alwaysRun = true) //uploads serverServer.zip file and verify  deployment
    public void jaggeryFileUpload() throws Exception {
        ...
        jaggeryAppUploaderClient
                .uploadJaggeryFile("serverServer.zip", FrameworkPathUtil.getSystemResourceLocation() + "artifacts" +
                        File.separator + "AS" + File.separator + "jaggery" + File.separator +
                        "serverServer.zip");
        JaggeryTestUtil.isJaggeryAppDeployed("serverServer", backendURL, sessionCookie);
        log.info("serverServer.zip file uploaded successfully");
    }

    @AfterClass(alwaysRun = true) // deletes the serverServer jaggery app
    public void jaggeryFileDelete() throws Exception {
        ...
        webAppAdminClient.deleteWebAppFile("serverServer", hostName);
        log.info("serverServer deleted successfully");
    }
    ...
    public void testApplication() throws Exception {
        ...
        URL jaggeryURL = new URL(getWebAppURL(WebAppTypes.JAGGERY) + "/serverServer/index.jag");
        ...
        String expectedJsonString = "{\"authenticate\" : true}";
