...
/**
 * This class sends requests to registry/index.jag and validates the response
 */
public class WSAS1886RegistryTest extends ASIntegrationTest {

    private static final Log log = LogFactory.getLog(WSAS1886RegistryTest.class);
    private final String hostName = "localhost";

    @BeforeClass(alwaysRun = true) // uploads registry.zip file and verify  deployment
    public void jaggeryFileUpload() throws Exception {
        ...
        JaggeryApplicationUploaderClient jaggeryAppUploaderClient = new JaggeryApplicationUploaderClient(backendURL,
                sessionCookie);
        jaggeryAppUploaderClient.uploadJaggeryFile("registry.zip",
                FrameworkPathUtil.getSystemResourceLocation() + "artifacts" + File.separator + "AS" + File.separator
                        + "jaggery" + File.separator + "registry.zip");
        JaggeryTestUtil.isJaggeryAppDeployed("registry", backendURL, sessionCookie);
        log.info("registry.zip file uploaded successfully");
    }

    @AfterClass(alwaysRun = true) // deletes the registry jaggery app
    public void jaggeryFileDelete() throws Exception {
        ...
        webAppAdminClient.deleteWebAppFile("registry", hostName);
    ...
    public void testApplication() throws Exception {
        ...
        URL jaggeryURL = new URL(getWebAppURL(WebAppTypes.JAGGERY) + "/registry/index.jag");
        ...
        String expectedJsonString = "{\"registryExistence\" : true, \"registryTags\" :[\"tag1\","
                + " \"tag2\"], \"resourceRating\" : {\"average\" : 3, \"user\" : 3}}";
