...
public class CdiProducesTestCase extends ASIntegrationTest {
    ...
    public CdiProducesTestCase(TestUserMode userMode) {
    ...
    @Test(groups = "wso2.as", description = "test cdi produce with servlet")
    public void testCdiServlet() throws Exception {
        ...
        assertTrue(result.startsWith("Hi, greetings from implementation one"),
                "Response doesn't contain the greeting, hi, of the url " + webAppURL);
        assertTrue(result.contains("Bye !"),
                "Response doesn't contain the greeting, bye, of the url " + webAppURL);
    }
