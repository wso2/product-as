...
public abstract class LazyLoadingBaseTest extends ASIntegrationTest {
    ...
    private static final String TENANT_INFO_SERVICE_ARTIFACT_LOCATION = ARTIFACTS_LOCATION +
            TENANT_INFO_SERVICE_FILE_NAME;
    ...
    public void init() throws Exception {
        ...
        TENANT_IDLE_TIME = Long.parseLong(asServer.getConfigurationNode(TENANT_IDLE_XPATH).getNodeValue())
                * 60 * 1000;
        WEB_APP_IDLE_TIME = Long.parseLong(asServer.getConfigurationNode(WEB_APP_IDLE_XPATH).getNodeValue())
                * 60 * 1000;

        File sourceFile = new File(CARBON_ARTIFACT_LOCATION);
        File targetFile = new File(CARBON_REPOSITORY_LOCATION);

        SUPER_TENANT_DOMAIN = new AutomationContext(PRODUCT_GROUP_NAME, INSTANCE_NAME, SUPER_TENANT_DOMAIN_KEY,
                USER_KEY).getSuperTenant().getDomain();
        TENANT_DOMAIN_1 = new AutomationContext(PRODUCT_GROUP_NAME, INSTANCE_NAME, TENANT_DOMAIN_1_kEY, USER_KEY).
                getContextTenant().getDomain();
        TENANT_DOMAIN_2 = new AutomationContext(PRODUCT_GROUP_NAME, INSTANCE_NAME, TENANT_DOMAIN_2_KEY, USER_KEY).
                getContextTenant().getDomain();
    ...
    protected TenantStatus getTenantStatus(String tenantDomain) throws LazyLoadingTestException {
        ...
        try {
            ...
            tenantStatus = new TenantStatus(tenantStatusJSON.getJSONObject("TenantStatus").
                    getBoolean("tenantContextLoaded"));
        ...
        while (((time = (Calendar.getInstance().getTimeInMillis() - startTime.getTimeInMillis())) < deploymentDelay)
            ...
            try {
                ...
                String customErrorMessage = "InterruptedException occurs when sleeping 1000 milliseconds and while" +
                        " waiting for Jaggery Application to get deployed " + interruptedException.getMessage();
                log.error(customErrorMessage);
                throw new LazyLoadingTestException(customErrorMessage, interruptedException);
            }
        }
        ...
        while (((time = (System.currentTimeMillis() - startTime)) < deploymentDelay) && doLoop) {
            try {
                ...
                String customErrorMessage = "ApplicationAdminExceptionException Exception when when calling " +
                        ...
                        applicationAdminExceptionException.getMessage();
                log.error(customErrorMessage);
                throw new LazyLoadingTestException(customErrorMessage, applicationAdminExceptionException);
            } catch (RemoteException remoteException) {
                String customErrorMessage = "RemoteException Exception when calling listAllApplications() methods in " +
                        "WebAppAdminClient\n" +
                        remoteException.getMessage();
                log.error(customErrorMessage);
                throw new LazyLoadingTestException(customErrorMessage, remoteException);
    ...
    protected void loginAsTenantAdmin(String domainKey) throws LazyLoadingTestException {
        try {
            AutomationContext automationContext = new AutomationContext("AS", "appServerInstance0001", domainKey,
                    "admin");
            ...
            String customErrorMessage = "XPathExpressionException exception  when login as tenant admin." +
                    xPathExpressionException.getMessage();
            log.error(customErrorMessage);
            ...
            String customErrorMessage = "LoginAuthenticationExceptionException exception  when login as tenant admin." +
                    loginAuthenticationExceptionException.getMessage();
            log.error(customErrorMessage);
        ...
        while (System.currentTimeMillis() - startTime < MAX_THRESHOLD_TIME) {
            ...
            if (isTenantInGhostState) {
                ...
                try {
                    ...
                    String customErrorMessage = "InterruptedException occurs when sleeping 1000 milliseconds and while " +
                            "waiting for Web-app to auto unload" + interruptedException.getMessage();
                    log.error(customErrorMessage);
                    throw new LazyLoadingTestException(customErrorMessage, interruptedException);
                }
            }
